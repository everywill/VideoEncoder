...
WASM_LIBX_SETTINGS=$(LIBX_SETTINGS) --disable-asm --host=x86-none-gnu
...
wasm-example:
	emcc --bind -I$(WASM_BUILD_DIR)/include webcodec.cpp $(WASM_BUILD_DIR)/lib/libx264.a $(SIMD_FLAGS) -O0 -s WARN_ON_UNDEFINED_SYMBOLS=0 -s EXPORT_ES6=1 -s INITIAL_MEMORY=1024MB  -Wall -lm -o x264.js
...
wasm-libx264-simd:
	cd ./x264 && \
	emconfigure ./configure $(WASM_INSTALLS) \
		--extra-cflags="-c $(SIMD_FLAGS) -D__REPLACE_SIMD__" \
		$(WASM_LIBX_SETTINGS) && \
	emmake make -j8 && \
	emmake make install
